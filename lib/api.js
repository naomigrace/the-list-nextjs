...
export async function createAction(jwt, formData) {
  const data = fetchAPI(
    jwt,
    `
    mutation addAction($input: createActionInput!) {
...
export async function getAllNeighborhoodNamesWithActions(jwt) {
  const data = fetchAPI(
    jwt,
    `
  {
...
export async function getActionsByCategory(jwt, category, preview) {
  ...
  query ActionsByCategory($where: JSON) {
    actions(sort: "date:desc", where: $where) {
      title
      slug
      description
      date
